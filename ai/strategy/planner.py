...
from ai.actions.write_plan import (
    WritePlan,
    update_plan_from_rsp,
    precheck_update_plan_from_rsp,
)
from ai.actions.ask_review import AskReview, ReviewConst
from ai.schema.plan import Plan, TaskResult, Task
from ai.schema.message import Message
from ai.schema.memory import Memory
from ai.schema.task_type import TaskType
...
class Planner(BaseModel):
    ...
    def __init__(self, goal: str = "", plan: Optional[Plan] = None, **kwargs):
    ...
    def get_plan_status(self, exclude: List[str] = None) -> str:
        ...
        prompt = PLAN_STATUS.format(
            ...
            task_results=(
                task_results if "task_result" not in exclude else exclude_prompt
            ),
            current_task=self.current_task.instruction,
            current_task_code=(
                self.current_task.code if "code" not in exclude else exclude_prompt
            ),
            current_task_result=(
                self.current_task.result
                if "task_result" not in exclude
                else exclude_prompt
            ),
            guidance=guidance,
        )
