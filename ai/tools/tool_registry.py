...
from utils.constants import TOOL_SCHEMA_PATH
from utils.logs import logger

from questin.tools.tool_convert import (
    convert_code_to_tool_schema,
    convert_code_to_tool_schema_ast,
)
from questin.tools.tool_data_type import Tool, ToolSchema


class ToolRegistry(BaseModel):
    tools: dict = {}
    tools_by_tags: dict = defaultdict(
        dict
    )  # two-layer k-v, {tag: {tool_name: {...}, ...}, ...}
            ...
            if ":" in key:
                ...
                for method_name in method_names:
                    if method_name in class_tool.schemas["methods"]:
                        methods_filtered[method_name] = class_tool.schemas["methods"][
                            method_name
                        ]
                    else:
                        logger.warning(
                            f"invalid method {method_name} under tool {class_tool_name}, skipped"
                        )
