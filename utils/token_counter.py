...
TOKEN_COSTS = {
    ...
    "gpt-4o-mini-transcribe": {"prompt": 0.00125, "completion": 0.005},
    "gpt-4o-mini-tts": {"prompt": 0.0006, "completion": 0.012},
    ...
    "text-embedding-ada-002": {"prompt": 0.00005, "completion": 0.0},
    "text-embedding-3-small": {"prompt": 0.00001, "completion": 0.0},
    "text-embedding-3-large": {"prompt": 0.000065, "completion": 0.0},
    "glm-3-turbo": {
        "prompt": 0.0007,
        "completion": 0.0007,
    ...
    "llama-3.3-70b-versatile": {"prompt": 0.00059, "completion": 0.00079},  # For groq
    "llama-3.1-8b-instant": {"prompt": 0.00005, "completion": 0.00008},  # For groq
    ...
    "deepseek-chat": {
        "prompt": 0.00056,
        "completion": 0.00168,
    },  # In deepseek official website, use deepseekV3.1-terminus
...
def count_message_tokens(messages, model="gpt-3.5-turbo-0125"):
    ...
    if model in {
        ...
        """
        For Llama models (including meta-llama/llama-4-maverick-17b-128e-instruct).
        Use cl100k_base encoding as approximation since Llama uses different tokenizer.
        """
        tokens_per_message = 3  # approximate message overhead
        tokens_per_name = 1
        # Use cl100k_base as fallback encoding for Llama models
        try:
            encoding = tiktoken.get_encoding("cl100k_base")
        except Exception:
            logger.warning(
                f"Could not get encoding for Llama model {model}, using cl100k_base"
            )
            encoding = tiktoken.get_encoding("cl100k_base")
    else:
...
def count_output_tokens(string: str, model: str) -> int:
    ...
    try:
        if model == "gpt-4.1-nano":
            ...
            # For Llama models, use cl100k_base as approximation
            logger.info(f"Using cl100k_base encoding for Llama model: {model}")
            encoding = tiktoken.get_encoding("cl100k_base")
        else:
            encoding = tiktoken.encoding_for_model(model)
    except KeyError:
        ...
        encoding = tiktoken.get_encoding("cl100k_base")
    logger.info(f"Model {model} encoding: {encoding.name}")
