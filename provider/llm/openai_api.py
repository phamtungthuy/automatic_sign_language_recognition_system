...
from utils.constants import USE_CONFIG_TIMEOUT, LLM_THINK_LOG
...
class OpenAILLM(BaseLLM):
    ...
    async def _achat_completion_stream(
        ...
        async for chunk in response:
            ...
            if (
                ...
                and choice_delta.reasoning_content
            ):
                collected_reasoning_messages.append(
                    choice_delta.reasoning_content
                )  # for deepseek
                if LLM_THINK_LOG:
                    log_llm_stream(choice_delta.reasoning_content)
                continue
            if hasattr(choice_delta, "reasoning") and choice_delta.reasoning:
                collected_reasoning_messages.append(choice_delta.reasoning)
                if LLM_THINK_LOG:
                    log_llm_stream(choice_delta.reasoning)
                continue
            chunk_message = choice_delta.content or ""  # extract the message
            finish_reason = (
                choice0.finish_reason if hasattr(choice0, "finish_reason") else None
            )
    ...
    def _cons_kwargs(
        self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT, **extra_kwargs
    ) -> dict:
        kwargs = {
            "messages": messages,
        ...
        if (
            self.model
            and self.model.find("gpt-oss-20b") != -1
            and self.config.reasoning_effort
        ):
            kwargs["reasoning_effort"] = self.config.reasoning_effort
        if self.model and "o1-" in self.model:
    ...
    async def acompletion_text(
        self, messages: list[dict], stream=False, timeout=USE_CONFIG_TIMEOUT
    ) -> str:
    ...
    async def _achat_completion_function(
        ...
        kwargs = self._cons_kwargs(
            messages=messages, timeout=self.get_timeout(timeout), **chat_configs
        )
        rsp: ChatCompletion = await self.aclient.chat.completions.create(**kwargs)
        if rsp.usage:
            self._update_costs(rsp.usage)
    ...
    async def aask_code(
        self, messages: list[dict], timeout: int = USE_CONFIG_TIMEOUT, **kwargs
    ) -> dict:
    ...
    def healthy(self) -> bool:
        ...
        health_url = f"{base_domain}/health"

        timeout = self.get_timeout(
            USE_CONFIG_TIMEOUT
        )  # reuse existing timeout config method
        ...
        try:
            resp = requests.get(health_url, **req_kwargs)
